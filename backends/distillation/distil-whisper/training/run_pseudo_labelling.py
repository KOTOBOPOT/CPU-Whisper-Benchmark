    ...
    if _is_local_saved_dataset(dataset_name):
        if streaming:
            raise ValueError(
                ...
                "which cannot be streamed. Re-run with `--streaming False`."
            )
        ...
        if isinstance(dataset, DatasetDict):
            if split not in dataset:
                raise ValueError(
                    f"Split '{split}' not found in dataset at '{dataset_name}'. "
                    f"Available splits: {list(dataset.keys())}."
                )
            return dataset[split]
        logger.warning(
            "Dataset at '%s' does not expose multiple splits. Returning the stored dataset as-is.", dataset_name
        )
        return dataset
...
def main():
    ...
    for split in data_splits:
        with accelerator.main_process_first():
            raw_datasets[split] = _load_split(
