...
_DATASET_FROM_DISK_CACHE: Dict[str, Union[DatasetDict, datasets.Dataset]] = {}


def _is_local_saved_dataset(path: Optional[str]) -> bool:
    if path is None:
        return False
    dataset_path = Path(path)
    if not dataset_path.exists() or not dataset_path.is_dir():
        return False
    markers = ["dataset_dict.json", "dataset_info.json", "data-00000-of-00001.arrow"]
    return any((dataset_path / marker).exists() for marker in markers)
...
def _load_split(
    ...
    **kwargs,
    ...
    if _is_local_saved_dataset(dataset_name):
        if streaming:
            raise ValueError(
                f"Dataset path '{dataset_name}' points to a local dataset saved with `save_to_disk`, "
                "which cannot be used with `--streaming True`. Disable streaming for local datasets."
            )
        if dataset_name not in _DATASET_FROM_DISK_CACHE:
            _DATASET_FROM_DISK_CACHE[dataset_name] = load_from_disk(dataset_name)
        dataset = _DATASET_FROM_DISK_CACHE[dataset_name]
        if isinstance(dataset, DatasetDict):
            if split is None:
                raise ValueError(
                    f"No split specified when loading dataset at '{dataset_name}'. "
                    f"Available splits: {list(dataset.keys())}."
                )
            if split not in dataset:
                raise ValueError(
                    f"Split '{split}' not found in dataset at '{dataset_name}'. Available splits: {list(dataset.keys())}."
                )
            return dataset[split]
        if split not in (None, "train"):
            logger.warning(
                "Dataset at '%s' does not expose the requested split '%s'. Returning the stored dataset as-is.",
                dataset_name,
                split,
            )
        return dataset
    ...
    if training_args.do_eval:
        ...
        if len(dataset_names_dict) == 1:
            ...
            for dataset_dict in dataset_names_dict:
                ...
                raw_datasets[pretty_name] = _load_split(
                    dataset_dict["name"],
                    dataset_dict["config"],
                    split=dataset_dict["split"],
                    streaming=data_args.streaming,
